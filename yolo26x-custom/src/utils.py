"""Helper utilities shared across the YOLO26x pipeline."""
...
def get_logger(name: str = "yolo26x", level: int = logging.INFO) -> logging.Logger:
...
def auto_device(preferred: Optional[str] = None) -> str:
    ...
    Returns:
        ``"0"`` if a CUDA GPU is available, ``"mps"`` on Apple silicon,
        otherwise ``"cpu"``. If ``preferred`` is supplied and valid it
...
def hash_directory(path: str | os.PathLike, algorithm: str = "sha256") -> str:
    """Compute a deterministic hash over the byte content of all files in a directory.

    Useful for lightweight dataset versioning when DVC is not desired.
    """
    p = Path(path)
    if not p.exists():

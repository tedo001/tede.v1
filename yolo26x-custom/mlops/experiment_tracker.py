"""Thin MLflow wrapper used by the training pipeline.

Falls back to no-ops if MLflow is not installed so that training itself
never fails because of optional tracking dependencies.
"""
...
LOGGER = get_logger("mlops.tracker")
...
class ExperimentTracker:
    ...
    def __init__(
        self,
        experiment_name: str = "yolo26x_custom",
        tracking_uri: Optional[str] = None,
    ) -> None:
        """Configure MLflow.

        Args:
            experiment_name: MLflow experiment to log into; created if missing.
            tracking_uri: Override the MLflow tracking URI; defaults to
    ...
    def enabled(self) -> bool:
        """Whether MLflow is loaded and tracking calls will be honoured."""
    ...
    def log_params(self, params: Dict[str, Any]) -> None:
        """Log a dict of (potentially nested) hyperparameters."""
        if not self.enabled:
            return
        flat: Dict[str, Any] = {}
    ...
    def log_artifact(self, path: str, artifact_path: Optional[str] = None) -> None:
        ...
        if not Path(path).exists():
            LOGGER.warning("Artifact path missing: %s", path)
            return
        try:
    ...
    def run(self, run_name: Optional[str] = None) -> Iterator["ExperimentTracker"]:
        """Context manager: ``with tracker.run("exp1"): ...``."""

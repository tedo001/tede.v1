"""Filesystem-backed model registry with semantic versioning."""
...
LOGGER = get_logger("tede.registry")
...
class ModelRegistry:
    ...
    def __init__(self, root: str = "model_registry", model_name: str = "tede") -> None:
        self.root = Path(root)
    ...
    def _next_version(self, bump: str = "minor") -> str:
        versions = self._read_index().get("versions", [])
        if not versions:
            return "v1.0"
        latest = versions[-1]["version"]
        m = _VERSION_RE.match(latest)
        major, minor = (int(m.group(1)), int(m.group(2))) if m else (1, 0)
        return f"v{major + 1}.0" if bump == "major" else f"v{major}.{minor + 1}"
